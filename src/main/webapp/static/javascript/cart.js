...
//swaped api functions

const extractProductsFromHTML = () => {
    const productsContainer = document.getElementById("products");
    const productElements = productsContainer.getElementsByClassName("product");

    for (const productElement of productElements) {
        const id = productElement.querySelector("button").getAttribute("data-product-id");
        const title = productElement.querySelector("h3").innerText;
        const image = productElement.querySelector("img").getAttribute("src");
        const priceText = productElement.querySelector("h5").innerText;

        // Extract numerical value from priceText (assuming it's in the format "X USD")
